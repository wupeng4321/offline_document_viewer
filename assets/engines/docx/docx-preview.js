(function (global, factory) {
    ...
    class xmlUtil {
        ...
        static colorAttr(node, attrName, defValue = null, autoColor = 'black') {
            ...
            /* PATCH: Word tema rengini <w:color w:val="auto" w:themeColor="accent1"/>
               seklinde yaziyor. Burada "auto" gorulunce hemen siyaha donuluyor ve
               tema dalina hic girilmiyordu -> tum tema METIN renkleri siyah cikiyordu.
               Tema referansi varsa "auto" bir deger degil, "temadan al" demektir. */
            var hasTheme = globalXmlParser.attr(node, "themeColor")
                || globalXmlParser.attr(node, "themeFill");
            if (v && !(v == "auto" && hasTheme)) {
                if (v == "auto") {
                    return autoColor;
                }
            ...
            /* PATCH: iki eksik.
               1) Tablo golgelendirmesi w:shd uzerinde themeFill kullaniyor ama
                  burada yalnizca themeColor araniyordu -> golgelendirme kayboluyordu.
               2) themeTint / themeShade hic uygulanmiyordu; "accent1, %40 acik"
                  duz accent1 olarak ciziliyordu. Tint/shade 0-255 arasi HEX;
                  tint beyaza, shade siyaha karistirma orani. color-mix() ile
                  CSS degiskenini bozmadan uyguluyoruz. */
            var themeColor = globalXmlParser.attr(node, "themeColor")
                || globalXmlParser.attr(node, "themeFill");
            if (!themeColor)
                return defValue;
            var base = `var(--docx-${themeColor}-color)`;
            var tint = globalXmlParser.attr(node, "themeTint")
                || globalXmlParser.attr(node, "themeFillTint");
            var shade = globalXmlParser.attr(node, "themeShade")
                || globalXmlParser.attr(node, "themeFillShade");
            if (tint) {
                var tp = Math.round(parseInt(tint, 16) / 255 * 100);
                if (!isNaN(tp) && tp < 100)
                    return `color-mix(in srgb, ${base} ${tp}%, white)`;
            }
            if (shade) {
                var sp = Math.round(parseInt(shade, 16) / 255 * 100);
                if (!isNaN(sp) && sp < 100)
                    return `color-mix(in srgb, ${base} ${sp}%, black)`;
            }
            return base;
        }
