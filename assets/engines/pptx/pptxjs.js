...
(function ($) {
    $.fn.pptxToHtml = function (options) {
        ...
        function getSlideSizeAndSetDefaultTextStyle(zip) {
            //get app version
            /* PATCH: docProps/app.xml zorunlu varsayiliyordu. PowerPoint her zaman
               yaziyor ama LibreOffice, Google Slides ve python-pptx ciktilarinda bu
               parca ya da AppVersion alani eksik olabiliyor -> tum render "Cannot read
               properties of null" ile cokuyordu. Eksikse bilinmeyen surum kabul edip
               devam ediyoruz; app_verssion yalnizca kucuk davranis farklari icin. */
            var app = readXmlFile(zip, "docProps/app.xml");
            var app_verssion_str = (app && app["Properties"] && app["Properties"]["AppVersion"]) || "";
            app_verssion = parseInt(app_verssion_str);
            if (isNaN(app_verssion)) {
                app_verssion = 16;
                console.warn("docProps/app.xml yok veya AppVersion okunamadi; 16 varsayildi");
            }
        ...
        /* PATCH: OOXML relationship targets may be package-absolute
           (/ppt/...) or relative to the owning part (../slideLayouts/...).
           PPTXjs only handled the latter and passed leading-slash paths to
           JSZip, which returns null. Normalize both forms before lookup. */
        function resolveRelationshipTarget(target, baseDirectory) {
            if (!target) {
                return "";
            }

            var normalizedTarget = String(target).replace(/\\/g, "/");
            if (normalizedTarget.charAt(0) == "/") {
                return normalizedTarget.replace(/^\/+/, "");
            }

            var result = [];
            var parts = (baseDirectory + normalizedTarget).split("/");
        ...
        function directoryOf(filename) {
            ...
            return separator == -1 ? "" : filename.substring(0, separator + 1);
        }
        ...
        function processSingleSlide(zip, sldFileName, index, slideSize) {
            ...
            */
            ...
            if (RelationshipArray.constructor === Array) {
                for (var i = 0; i < RelationshipArray.length; i++) {
                    switch (RelationshipArray[i]["attrs"]["Type"]) {
                        case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout":
                            layoutFilename = resolveRelationshipTarget(RelationshipArray[i]["attrs"]["Target"], "ppt/slides/");
                            break;
                        case "http://schemas.microsoft.com/office/2007/relationships/diagramDrawing":
                            diagramFilename = resolveRelationshipTarget(RelationshipArray[i]["attrs"]["Target"], "ppt/slides/");
                            slideResObj[RelationshipArray[i]["attrs"]["Id"]] = {
                                "type": RelationshipArray[i]["attrs"]["Type"].replace("http://schemas.openxmlformats.org/officeDocument/2006/relationships/", ""),
                                "target": resolveRelationshipTarget(RelationshipArray[i]["attrs"]["Target"], "ppt/slides/")
                            };
                            break;
                ...
                layoutFilename = resolveRelationshipTarget(RelationshipArray["attrs"]["Target"], "ppt/slides/");
            }
                ...
                for (var i = 0; i < RelationshipArray.length; i++) {
                    switch (RelationshipArray[i]["attrs"]["Type"]) {
                        case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideMaster":
                            masterFilename = resolveRelationshipTarget(RelationshipArray[i]["attrs"]["Target"], "ppt/slideLayouts/");
                            break;
                        default:
                            layoutResObj[RelationshipArray[i]["attrs"]["Id"]] = {
                                "type": RelationshipArray[i]["attrs"]["Type"].replace("http://schemas.openxmlformats.org/officeDocument/2006/relationships/", ""),
                                "target": resolveRelationshipTarget(RelationshipArray[i]["attrs"]["Target"], "ppt/slideLayouts/")
                            };
                    }
                }
            } else {
                masterFilename = resolveRelationshipTarget(RelationshipArray["attrs"]["Target"], "ppt/slideLayouts/");
            }
                ...
                for (var i = 0; i < RelationshipArray.length; i++) {
                    switch (RelationshipArray[i]["attrs"]["Type"]) {
                        case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
                            themeFilename = resolveRelationshipTarget(RelationshipArray[i]["attrs"]["Target"], "ppt/slideMasters/");
                            break;
                        default:
                            masterResObj[RelationshipArray[i]["attrs"]["Id"]] = {
                                "type": RelationshipArray[i]["attrs"]["Type"].replace("http://schemas.openxmlformats.org/officeDocument/2006/relationships/", ""),
                                "target": resolveRelationshipTarget(RelationshipArray[i]["attrs"]["Target"], "ppt/slideMasters/")
                            };
                    }
                }
            } else {
                themeFilename = resolveRelationshipTarget(RelationshipArray["attrs"]["Target"], "ppt/slideMasters/");
            }
                        ...
                        if (relationshipArray.constructor === Array) {
                            for (var i = 0; i < relationshipArray.length; i++) {
                                themeResObj[relationshipArray[i]["attrs"]["Id"]] = {
                                    "type": relationshipArray[i]["attrs"]["Type"].replace("http://schemas.openxmlformats.org/officeDocument/2006/relationships/", ""),
                                    "target": resolveRelationshipTarget(relationshipArray[i]["attrs"]["Target"], directoryOf(themeFilename))
                                };
                            }
                        } else {
                            ...
                            themeResObj[relationshipArray["attrs"]["Id"]] = {
                                ...
                                "target": resolveRelationshipTarget(relationshipArray["attrs"]["Target"], directoryOf(themeFilename))
                            };
                        }
                    }
                }
                    ...
                    if (relationshipArray.constructor === Array) {
                        for (var i = 0; i < relationshipArray.length; i++) {
                            diagramResObj[relationshipArray[i]["attrs"]["Id"]] = {
                                "type": relationshipArray[i]["attrs"]["Type"].replace("http://schemas.openxmlformats.org/officeDocument/2006/relationships/", ""),
                                "target": resolveRelationshipTarget(relationshipArray[i]["attrs"]["Target"], directoryOf(diagramFilename))
                            };
                        }
                    } else {
                        //console.log("theme relationshipArray : ", relationshipArray)
                        diagramResObj[relationshipArray["attrs"]["Id"]] = {
                            "type": relationshipArray["attrs"]["Type"].replace("http://schemas.openxmlformats.org/officeDocument/2006/relationships/", ""),
                            "target": resolveRelationshipTarget(relationshipArray["attrs"]["Target"], directoryOf(diagramFilename))
                        };
                    }
                }
            }
        ...
        */
        ...
        function genTextBody(textBodyNode, spNode, slideLayoutSpNode, slideMasterSpNode, type, idx, warpObj, tbl_col_width) {
            ...
            /* PATCH: <a:bodyPr numCol="2"> (cok sutunlu metin kutusu) ve
               <a:bodyPr vert="vert|vert270"> (dikey metin) hic islenmiyordu; ikisi de
               sessizce yok sayilip metin tek sutun ve yatay ciziliyordu. */
            var bodyPrAttrs = getTextByPathList(textBodyNode, ["a:bodyPr", "attrs"]) || {};
            var bodyWrapStyle = "";
            var numCol = parseInt(bodyPrAttrs["numCol"], 10);
            if (numCol > 1) {
                var spcCol = parseInt(bodyPrAttrs["spcCol"], 10);
                /* column-fill:balance sart: sarmalayicida kesin bir yukseklik
                   varken tarayici once 1. sutunu doldurup ikinciyi bos birakiyor.
                   PowerPoint sutunlari dengeler. */
                bodyWrapStyle += "column-count:" + numCol + ";column-fill:balance;";
                if (!isNaN(spcCol)) {
                    bodyWrapStyle += "column-gap:" + Math.round(spcCol * slideFactor) + "px;";
                }
            }
            var vert = bodyPrAttrs["vert"];
            if (vert === "vert") {
                bodyWrapStyle += "writing-mode:vertical-rl;text-orientation:mixed;";
            } else if (vert === "vert270") {
                bodyWrapStyle += "writing-mode:vertical-rl;transform:rotate(180deg);";
            } else if (vert === "wordArtVert") {
                bodyWrapStyle += "writing-mode:vertical-rl;text-orientation:upright;";
            }
            ...
            if (bodyWrapStyle) {
                /* Sutunlara bolunebilmesi icin ic paragraf div'lerinin sabit
                   width'i gevsetilmeli — PPTXjs her paragrafa width:NNNpx yaziyor
                   ve sabit genislikli blok sutuna sigmiyor. */
                ensureMultiColStyle();
                text = "<div class='pptxjs-bodywrap' style='"
                    + bodyWrapStyle + "'>" + text + "</div>";
            }
            return text;
        }
        ...
        function genBuChar(node, i, spNode, textBodyNode, pFontStyle, idx, type, warpObj) {
            ...
            if (buType == "TYPE_BULLET") {
                ...
                /* PATCH: buFont sadece paragrafin kendi pPr'inda araniyordu; layout ve
                   master'dan miras alinan madde imlerinde undefined kaliyor ve Wingdings
                   karakteri PUA kod noktasi olarak (tofu) basiliyordu. buFontSize icin
                   zaten uygulanan miras zincirinin aynisi. */
                if (typefaceNode === undefined) {
                    typefaceNode = getTextByPathList(pPrNodeLaout, ["a:buFont", "attrs", "typeface"]);
                }
                if (typefaceNode === undefined) {
                    typefaceNode = getTextByPathList(pPrNodeMaster, ["a:buFont", "attrs", "typeface"]);
                }
                var typeface = "";
                if (typefaceNode !== undefined) {
        ...
        /* PATCH: PPTXjs'in XML ayristiricisi attribute degerlerindeki sayisal
           karakter referanslarini (&#xF097; / &#61591;) cozmuyor. Boyle yazilmis
           bir madde imi geldiginde ilk karakter '&' (0x26) olarak okunuyor ve
           dingbat tablosunda tamamen alakasiz bir sembole dusuyor. PowerPoint
           ham karakter yaziyor ama LibreOffice / python-pptx / elle uretilen
           dosyalar referans kullanabiliyor. */
        var _multiColStyleAdded = false;
        function ensureMultiColStyle() {
            if (_multiColStyleAdded) return;
            _multiColStyleAdded = true;
            var st = document.createElement("style");
            st.textContent =
                /* dogrudan cocuk degil: PPTXjs paragraflari ic ice div'lere
                   sariyor ve sabit width en icteki div'de olabiliyor */
                /* dogrudan cocuk degil: PPTXjs paragraflari ic ice div'lere sariyor.
                   display:flex de gerekli — flex satiri sutunlara bolunmuyor.
                   Bedeli: cok sutunlu kutuda madde imi hizasi bozulabilir. */
                ".pptxjs-bodywrap div { width: auto !important; display: block !important; }" +
                ".pptxjs-bodywrap { break-inside: auto; }";
            document.head.appendChild(st);
        }

        function decodeCharRef(ch) {
            if (typeof ch !== "string") return ch;
            var m = /^&#(x[0-9a-fA-F]+|\d+);$/.exec(ch.trim());
            if (!m) return ch;
            var code = m[1][0] === "x" || m[1][0] === "X"
                ? parseInt(m[1].slice(1), 16)
                : parseInt(m[1], 10);
            return isNaN(code) ? ch : String.fromCodePoint(code);
        }

        function getHtmlBullet(typefaceNode, buChar) {
            buChar = decodeCharRef(buChar);
            ...
            switch (buChar) {
                ...
                case "ü":
                    ...
                    break;
                default:
                    if (typefaceNode == "Wingdings" || typefaceNode == "Wingdings 2" || typefaceNode == "Wingdings 3" || typefaceNode == "Webdings"){ /* PATCH: Wingdings ve Webdings de dingbat tablosundan gecsin */
